...
export function NewSaleTab() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (activeCategory !== "All") {
      ...
      const targetType =
        typeMap[activeCategory] || activeCategory.toLowerCase();
  ...
  const totals = useMemo(() => {
    const subtotal = Array.isArray(basket)
      ? basket.reduce((sum, i) => sum + i.unitPrice * i.quantity, 0)
      : 0;
    const itemDiscounts = Array.isArray(basket)
      ? basket.reduce((sum, i) => sum + i.discount, 0)
      : 0;
  ...
  const handleComplete = async () => {
    ...
    if (totals.balance > 0)
      return messageApi.warning("Remaining balance must be zero");
              ...
              >
                <div className="flex flex-col h-full justify-between gap-2">
                  <div>
                    ...
                    <Text
                      strong
                      className="text-[14px] line-clamp-2 leading-tight h-10 group-hover:text-blue-700 transition-colors"
                    >
                      {item.name}
                    </Text>
                  </div>
      ...
      <div className="flex-1 flex flex-col bg-blue-50/20 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex flex-col">
            <div className="flex justify-between items-end mb-3 px-1">
              <div className="flex items-center gap-2">
                <Title
                  level={5}
                  className="!m-0 !text-[12px] font-black uppercase tracking-widest text-blue-800"
                >
                  Basket Summary
                </Title>
                <Badge
                  count={basket.length}
                  style={{ backgroundColor: "#1d4ed8", fontSize: "10px" }}
                />
              </div>
              <Button
                type="text"
                danger
                size="small"
                className="text-[10px] font-black uppercase p-0 h-auto opacity-70 hover:opacity-100"
                onClick={clearBasket}
              >
                Clear All
              </Button>
            ...
            <div
              ...
              className="h-[340px] overflow-y-auto bg-white rounded-2xl border border-blue-100 shadow-sm custom-scrollbar"
            >
              {basket.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-300 p-8 text-center bg-gray-50/20">
                  ...
                  />
                  <Text className="mt-4 text-xs font-medium text-gray-400 italic">
                    Select products or services to begin
                  </Text>
                </div>
              ) : (
                <div className="divide-y divide-gray-100">
                  {basket.map((line) => (
                    <div
                      key={line.id}
                      className="p-3 flex flex-col gap-2 hover:bg-blue-50/20 transition-colors border-l-4 border-transparent hover:border-blue-400"
                    >
                      <div className="flex justify-between items-start gap-2">
                        <Text
                          strong
                          className="text-[13px] text-gray-800 leading-tight flex-1"
                        >
                          {line.name}
                        </Text>
                        <Button
                      ...
                      <div className="flex items-center justify-between bg-gray-50/50 px-2 py-1.5 rounded-lg border border-gray-100/50">
                        <div className="flex items-center gap-3">
                          <div className="flex flex-col">
                            <Text className="text-[8px] uppercase font-black text-gray-400">
                              Rate
                            </Text>
                            <Text className="text-[11px] font-mono font-bold text-gray-600">
                          ...
                          <div className="flex flex-col">
                            ...
                            <InputNumber
                              ...
                              value={line.quantity}
                              onChange={(v) =>
                                updateQuantity(line.itemId, v || 1)
                              }
                        ...
                        <div className="text-right">
                          <Text className="text-[8px] uppercase font-black text-gray-400 block">
                            Total
                          </Text>
                          <Text
                            strong
              ...
              {(Array.isArray(payments) ? payments : []).map((p, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <Text className="text-[12px] capitalize font-black text-gray-700">
                      {p.method}
                    </Text>
                  </div>
                  <div className="flex items-center gap-3">
                    <Text
                      strong
                      className="text-[13px] font-black text-blue-900"
                    >
          ...
          <div className="bg-blue-900 text-white rounded-2xl p-6 shadow-2xl shadow-blue-200 relative overflow-hidden border border-blue-800">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="flex justify-between items-center text-[13px] font-bold border-b border-white/20 pb-4">
                ...
                <Text className="!text-orange-200 font-mono">
                  -₦{totals.itemDiscounts?.toLocaleString()}
                </Text>
              </div>
              ...
              <div className="flex justify-between items-center pt-2">
                <Title
                  level={4}
                  className="!m-0 !text-white !font-black !uppercase text-[14px]! !tracking-widest"
                >
                  Total Due
                </Title>
                <Title
                  level={1}
                  className="m-0! text-white! font-black! text-[36px]! drop-shadow-xl"
                >
        ...
        <div className="mt-auto px-6 py-4 bg-white border-t-2 border-gray-100 flex flex-col gap-3 sticky bottom-0 z-30 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex gap-3">
            <Button
              icon={<SaveOutlined />}
              className="flex-1 rounded-xl h-9 text-[11px] font-black uppercase border-gray-200 hover:text-blue-600 hover:border-blue-400 transition-colors"
              onClick={() => holdSale()}
            >
              Hold Order
            </Button>
            <Button
              icon={<StopOutlined />}
              className="flex-1 rounded-xl h-9 text-[11px] font-black uppercase border-gray-200 hover:text-red-600 hover:border-red-400 transition-colors"
              onClick={clearBasket}
            >
              Cancel
            </Button>
